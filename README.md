...
The helper asks OCR for the reviewable file set and applicable rules without invoking an LLM. With `--packets`, it also emits schema-2, deterministic rule-group packets containing bounded diff segments and changed-line anchors. Packets are capped at 128 KiB by default and oversized groups or files are split deterministically; adjust this with `--packet-max-bytes`. Codex then reviews those packets and emits structured finding JSON; `normalize_findings` rejects out-of-scope or unchanged-line findings, deduplicates, and sorts the final output. Schema-1 manifests remain valid inputs.
